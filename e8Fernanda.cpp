...
int buscaAresta(FILE* nova, int vertice1, int vertice2){
    struct registro arestas_binarias[MAX];
        printf("\nSeu novo arquivo possui os seguintes registros:\n");
        //printf("%c", fgetc(nova));
        int i = 0;
        while(1 == fread(&arestas_binarias[i], sizeof(REGISTRO), 1, nova)){
            printf("\%d", arestas_binarias[i].v1);
            printf("    %d", arestas_binarias[i].v2);
            printf("    %d", arestas_binarias[i].custo);
            if((arestas_binarias[i].v1 == vertice1)&&(arestas_binarias[i].v2 == vertice2)){
                return arestas_binarias[i].custo;
            }
            i++;
        }
...
int main(){
    ...
    if((arq = fopen("grafo.txt","rb")) == NULL){ //adicionar arquivo texto se quiser
        ...
        FILE* nova = NULL; 
        if((nova = fopen("teste1.oi","wb+")) == NULL){ //mudar o arquivo de grafico aqui
            ...
            int vertice1 = 0;
            int vertice2 = 0;
            printf("Qual aresta voce busca?");
            scanf("%d %d", &vertice1, &vertice2);  
            printf("Custo: %d\n", buscaAresta(nova, vertice1, vertice2));
        }
        printf("\nFim do processo");
    return 0;
}
